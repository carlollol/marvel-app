...
for (var i = 0; i < heroes.length; i++) {
    var start = $("<div class='panel panel-default'><div class='panel-heading'><h4 class='panel-title'><h4 data-toggle='collapse' data-parent='#accordion' href='#collapse"+i+"'>"+heroes[i].alias+"</h4></h4></div><div id='collapse"+i+"' class='panel-collapse collapse'><div class='panel-body' id='info"+i+"'><section><strong><h5>Name:</strong> "+heroes[i].name+"</h5><strong><h5>Powers: </strong>"+heroes[i].powers+"</h5><h5><strong>Location: </strong><span class='locInfo' id='locInfo"+i+"'>"+heroes[i].location+"<span></h5><video height='300px' muted loop controls='controls' preload='none' class='bgvid'><source src='"+heroes[i].video+"' type='video/mp4'></video></section></div></div></div>");
...
$("#submit_hero").on("click", function(event) {
    ...
    var heroLocation = $("#yourlocation").val().trim();
...
database.ref().on("child_added", function(childSnapshot, prevChildKey) {
    ...
    var heroLocation = childSnapshot.val().location;
    ...
    $("#accordion").append(start);
      ...
      function initMap() {
        var map = new google.maps.Map(document.getElementById('starter-rh'), {
          zoom: 13,
        ...
        $(document).on('click', '.locInfo', function(e){
            var id = (this.id).substr(7,1);
            var address = heroes[id].location;
            geocodeDoIt(geocoder, map, address);
        });
      // $("#locInfo0").on("click", function (){
      ...
      function geocodeDoIt(geocoder, resultsLocation, address) {
        // var address = document.getElementById('locInfo0').value;
        // var opM = $("#locInfo0")
        // console.log(opM)
        console.log('this is the address that got passed on to geoCode');
        console.log(address);
        geocoder.geocode({'address': address}, function(results, status) {
          if (status === 'OK') {
            console.log(results);
